...
class HttpClient {
  ...
  constructor(
    ...
    private validateRequestUrl?: (url: URL) => void | Promise<void>,
  ...
  async request({
    ...
    await this.validateRequestUrl?.(new URL(requestUrl));
    const controller =
    ...
    try {
      response = await fetch(requestUrl, {
        method,
        redirect: redirect ?? (this.followRedirects ? "follow" : "manual"),
