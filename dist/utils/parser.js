...
const parseCalendars = async (responseData, baseUrl) => {
    ...
    for (const res of responses) {
        ...
        calendars.push({
            ...
            supportedComponents,
            color: prop === null || prop === void 0 ? void 0 : prop["calendar-color"],
        });
    }
    return calendars;
};
    ...
    for (const obj of response) {
        ...
        try {
            ...
            for (const vevent of vevents) {
                ...
                const rawStatus = (_k = vevent.getFirstPropertyValue("status")) === null || _k === void 0 ? void 0 : _k.toString();
                const status = models_1.EVENT_STATUSES.includes(rawStatus)
                    ? rawStatus
                    : undefined;
                events.push({
                    uid: icalEvent.uid,
    ...
    for (const obj of response) {
        ...
        try {
            ...
            for (const vtodo of vtodos) {
                ...
                const rawStatus = vtodo.getFirstPropertyValue("status");
                const status = models_1.TODO_STATUSES.includes(rawStatus)
                    ? rawStatus
                    : undefined;
